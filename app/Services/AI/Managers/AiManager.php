...
class AiManager
{
    public function getChatResponse(array $userMessages): array
    {
        ...
        $systemPrompt = "Ти — емпатійний AI-експерт платформи 'AdoptMe Dnipro'. 
            ...
            ОСЬ ГОЛОВНЕ ПРАВИЛО: Ти ПОВИНЕН відповідати ВИКЛЮЧНО у форматі JSON. Без жодного додаткового тексту чи markdown розмітки.
            Формат JSON:
            {
            \"reply\": \"Твій текст відповіді користувачу українською мовою\",
            \"suggested_pet_ids\": [масив ID тварин з переданого списку, які підходять під критерії. Якщо підходять всі або критеріїв ще немає — повертай порожній масив []]
            }
            Список доступних тварин: {$petsJson}";
